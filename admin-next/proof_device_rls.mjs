...
async function run() {
  ...
  let deviceUserId, deviceUserId2, orderId;

  // try/finally guarantees cleanup even when a test throws an unhandled exception.
  // Without this, a mid-run crash leaves orphan auth users and device rows in the
  // live database, and the partial unique index on devices(auth_user_id) blocks
  // the next run from creating the same test rows.
  try {
    ...
    console.log('\n── Test 7: Device can only see order_items for its own restaurant ────');
    {
      const { data: user2 } = await admin.auth.admin.createUser({
        email: testEmail2, password: testPass, email_confirm: true,
      });
      ...
      const { data: session3 } = await anon.auth.signInWithPassword({
        email: testEmail2, password: testPass,
      });
      ...
      if (itemErr) {
        ...
        pass('Device sees 0 order_items (no active orders in its restaurant, or RLS blocking)');
        console.log('  INFO  Place a pending order and re-run to confirm the positive case.');
      } else {
        ...
        if (foErr) {
          ...
          fail('Device can read order_items from another restaurant', JSON.stringify(foreignOrders));
        } else {
          pass(`All ${visibleItems.length} visible order_items belong to device restaurant`);
        }
      }
    }
    ...
    console.log('\n── Test 8: Anon cannot insert waiter_call for foreign restaurant_id ─');
    {
      ...
      if (otherRestaurants && otherRestaurants.length > 0) {
        ...
        console.log(`  INFO  Only one restaurant in DB — using fake UUID (FK catches RLS gaps)`);
      }
    ...
    // Runs whether tests pass, fail, or throw. Cascade on auth.users deletes devices rows.
    console.log('\n── Cleanup ──────────────────────────────────────────────────────────');
    if (deviceUserId2) {
      await admin.auth.admin.deleteUser(deviceUserId2).catch(() => {});
    }
    if (deviceUserId) {
      const { error } = await admin.auth.admin.deleteUser(deviceUserId);
      if (error) {
        console.error(`  WARN  Cleanup failed for ${deviceUserId}: ${error.message}`);
        console.error(`  WARN  Delete manually: supabase.auth.admin.deleteUser('${deviceUserId}')`);
      } else {
        pass('Test device auth users deleted');
      }
    }
  }
